...
static FrameCanvas *g_canvas =
    nullptr; // NOTE: FrameCanvas, not OffscreenCanvas
static Font g_font;
...
static void renderClear(FrameCanvas *c) { c->Fill(0, 0, 0); }
...
int main(int argc, char **argv) {
  ...
  // Keep running as root so we can open /dev/ttyUSB0 after matrix init.
  // By default the library drops to user "daemon", which has no serial access.
  runtimeOpts.drop_privileges = 0;
